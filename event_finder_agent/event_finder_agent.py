...
class EventFinderAgent:
    """Event Finder Agent for discovering weekend events."""

    SYSTEM_INSTRUCTION = """You are a specialized assistant for finding events and activities. Your primary function is to utilize the provided tools to search for events from sources like Eventbrite and The Boston Calendar for the upcoming weekend and answer related questions. You must rely exclusively on these tools for information; do not invent event data. Ensure that your Markdown-formatted response includes all relevant event information, focusing on weekend days (Saturday and Sunday)."""
    ...
    def __init__(self):
        """Initializes the Event Finder agent."""
        logger.info('Initializing EventFinderAgent...')
        try:
        ...
        # Initialize tools for event finding
        self.tools = [
            {
                "type": "function",
                "function": {
                    "name": "search_events",
                    "description": "Search for events in a specific location for the upcoming weekend",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "location": {
                                "type": "string",
                                "description": "The city and state/country for event search"
                            },
                            "category": {
                                "type": "string",
                                "description": "Optional category of events (e.g., music, food, sports)",
                                "enum": ["music", "food", "sports", "arts", "family", "all"]
                            }
                        },
                        "required": ["location"]
                    }
    ...
    async def ainvoke(self, query: str, session_id: str) -> dict[str, Any]:
        logger.info(
            f"EventFinderAgent.ainvoke called with query: '{query}', session_id: '{session_id}'"
        )
        try:
            event_finder_agent_runnable = create_react_agent(
                self.model,
            ...
            logger.debug(
                'LangGraph React agent for Event Finder task created/configured.'
            )
            ...
            logger.debug(
                f'Invoking Event Finder Agent with input: {langgraph_input} and config: {config}'
            )
            result = await event_finder_agent_runnable.ainvoke(langgraph_input, config)
            logger.debug(f'Raw result from Event Finder Agent: {result}')

            # Extract the response from the state
            response = self._get_agent_response_from_state(config, event_finder_agent_runnable)
            logger.info(
                f'Final response from Event Finder Agent for session {session_id}: {response}'
            )
            return response

        except Exception as e:
            logger.error(
                f'Error during EventFinderAgent.ainvoke for session {session_id}: {e}',
                exc_info=True,
            )
            return {
    ...
    def _get_tools(self):
        """Returns the tools for the Event Finder Agent."""
        
        def search_events(location: str, category: str = "all"):
            ...
            try:
                ...
                # In a real implementation, you would call an events API here (Eventbrite, etc.)
                # For now, we'll return mock data
                
                # Generate different events based on the category
                events = []
                
                if category == "music" or category == "all":
                    events.extend([
                        {
                            "name": "Weekend Jazz Festival",
                            "date": saturday.strftime("%Y-%m-%d"),
                            "time": "7:00 PM",
                            "location": f"{location} Music Hall",
                            "description": "A night of smooth jazz with local and international artists.",
                            "category": "music",
                            "price": "$25-45",
                            "link": "https://example.com/jazz-festival"
                        },
                        {
                            "name": "Sunday Symphony Orchestra",
                            "date": sunday.strftime("%Y-%m-%d"),
                            "time": "3:00 PM",
                            "location": f"{location} Concert Hall",
                            "description": "Classical music performance featuring Beethoven's 5th Symphony.",
                            "category": "music",
                            "price": "$30-60",
                    ...
                    events.extend([
                        {
                            ...
                            "location": f"{location} Downtown Park",
                            "description": "Over 20 food trucks offering cuisines from around the world.",
                            "category": "food",
                            "price": "Free entry, pay per item",
                            "link": "https://example.com/food-truck-fest"
                        }
                    ])
                ...
                if category == "sports" or category == "all":
                    events.extend([
                        {
                            "name": f"{location} Marathon",
                            "date": sunday.strftime("%Y-%m-%d"),
                            "time": "8:00 AM",
                            "location": f"{location} City Center",
                            "description": "Annual marathon through the scenic routes of the city.",
                            "category": "sports",
                ...
                if category == "arts" or category == "all":
                    events.extend([
                        {
                            "name": "Weekend Art Exhibition",
                            "date": f"{saturday.strftime('%Y-%m-%d')} - {sunday.strftime('%Y-%m-%d')}",
                            "time": "10:00 AM - 6:00 PM",
                            "location": f"{location} Art Gallery",
                ...
                if category == "family" or category == "all":
                    events.extend([
                        {
                            "name": "Family Fun Day",
                            "date": saturday.strftime("%Y-%m-%d"),
                            ...
                            "description": "Activities for the whole family including games, crafts, and entertainment.",
                            "category": "family",
                            "price": "$10 per family",
                            "link": "https://example.com/family-fun"
                        }
                    ])
    ...
    async def stream(self, query: str, session_id: str) -> AsyncIterable[Any]:
        logger.info(
            f"EventFinderAgent.stream called with query: '{query}', sessionId: '{session_id}'"
        )
        ...
        logger.debug(
            f'Streaming from Event Finder Agent with input: {langgraph_input} and config: {config}'
        )
        try:
            ...
            logger.error(
                f'Error during EventFinderAgent.stream for session {session_id}: {e}',
                exc_info=True,
            )
            yield {
                'is_task_complete': True,
